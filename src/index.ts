...
import { IssueOption, IssueResponse, ReportDict, TrivyIssue } from './interface.js' // Added TrivyIssue
...
async function main() {
  ...
  try {
    ...
    if (inputs.issue.createLabels) {
      ...
      if (inputs.issue.enableFixLabel && inputs.issue.fixLabel) { // Check if fixLabel exists
    ...
    const existingTrivyIssues: TrivyIssue[] = await github.getTrivyIssues(inputs.issue.labels) // Renamed variable
    ...
    for (const existingIssue of existingTrivyIssues) {
      ...
      if (existingIssue.state === 'open' && !reportIssueTitles.has(existingIssue.title.toLowerCase())) {
        if (inputs.dryRun) {
          core.info(
            `[Dry Run] Would close stale issue: #${existingIssue.number} - ${existingIssue.title}`
          );
        } else {
          core.info(`Closing stale issue: #${existingIssue.number} - ${existingIssue.title}`);
      ...
      if (existingIssue.state === 'open' && inputs.issue.enableFixLabel && inputs.issue.fixLabel && existingIssue.labels.includes(inputs.issue.fixLabel)) {
    ...
    if (reports === null) {
      ...
      if (!fixableVulnerabilityExists) { // Only check if not already found in existing issues
        fixableVulnerabilityExists = issuesToProcess.some(issue => issue.hasFix);
      }


      // Create/Update GitHub issues
      for (const issue of issuesToProcess) { // Use renamed variable
        const existingIssue = existingTrivyIssues.find( // Use renamed variable
          (ei) =>
            ei.title.toLowerCase() === issue.title.toLowerCase()
        )
    ...
    // --- Set Outputs ---
    core.setOutput('fixable_vulnerability', fixableVulnerabilityExists.toString()) // Set the new output
